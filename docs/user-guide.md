...
MotionSlice 完整支持多音轨专业视频，无需手动处理音频流：

**支持特性：**
- ✅ 自动保留所有音频流（如专业摄像设备录制的 8 条单声道音轨）
- ✅ 切分时不会丢失任何音轨，原样保留音频结构
- ✅ 自动将 PCM 音频转码为 AAC，确保 Windows 和 macOS 跨平台兼容

**适用场景：**
- 专业摄像设备（如 Sony）录制的多音轨 MOV 文件
- 包含多语言配音的视频
- 带有独立背景音乐轨道的视频

**技术说明：**
v0.0.7 版本修复了多音轨视频切分时音频流丢失的问题。现在所有音频流都会完整保留到导出文件中，无需担心声音缺失或只有单声道的情况。
...
**参数说明**：
- 缓冲时长：0.0 - 30.0 秒（推荐 1.0 - 2.0 秒，专业剪辑可设置 3-10 秒）
...
> **💡 配置自动保存**  
> v0.0.6 起，切片参数和导出设置会在 Tab 切换时自动保留，无需重复配置。如需重置为默认值，请重启应用。
...
- **100（无损）**：
  - 视频流直接拷贝，不重新编码
  - 音频流：MP4 格式会转码为 AAC（192k，确保兼容性），MOV/AVI 格式直接拷贝
  - 速度快，文件略大于原始大小
- **80-99**：轻微压缩，兼顾质量和大小
- **60-79**：标准压缩，适合网络分发
- **10-59**：高压缩，文件小但质量下降
...
- 切换模式时，**工作台中的切片预览**会被清空（单选模式的时间轴和列表 / 批量模式的切片组）
- **导出面板中的配置**（输出目录、格式、质量）会保留
- 已加入导出队列的任务不受影响
- 建议在开始工作前确定使用哪种模式
...
### Q8: 为什么导出的视频在 Windows 上听不到声音？

**答案（v0.0.7 已修复）**：

**历史问题：**
- v0.0.6 及更早版本只保留第一条音轨，多音轨视频切分后可能听不到声音
- 专业设备录制的 PCM 音频在 Windows 播放器中兼容性差

**v0.0.7 解决方案：**
- ✅ 自动保留所有音频流（8 条单声道音轨全部保留）
- ✅ 导出为 MP4 时，自动将 PCM 转码为 AAC（确保 Windows/Mac 兼容）
- ✅ 无需手动处理音频，导出后的视频在所有平台都能正常播放

**验证方法：**
如果你仍然遇到声音问题，请检查：
1. 确认应用版本是 v0.0.7 或更高
2. 使用 VLC 播放器查看音频流数量（工具 → 编解码器信息 → 音频轨道）
3. 如果只有 1 条音轨，请重新用最新版本导出
...
### v0.0.7 (2026-07-07)
- 🐛 修复多音轨视频切分时音频流丢失问题
- ✨ 自动保留所有音频轨道（如 8 条单声道音轨）
- ✨ PCM 音频自动转码为 AAC，提升跨平台兼容性

### v0.0.6 (2026-07-05)
- 🐛 修复 Tab 切换导致配置信息丢失问题
- 🎨 统一导出面板 UI，合并单选和批量模式显示
- 🐛 修复导出进度同步和失败状态显示问题
