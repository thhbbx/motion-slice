...
### 最终架构
```
.video-tool-page (100vh × 100vw, flex 容器)
├── .sidebar (260px, height: 100%)
...
在 `.video-tool-page` 内部添加三栏布局的主容器：
...
- [ ] **步骤 1：添加 Sidebar 面板容器和占位卡片**

在 `<aside class="sidebar">` 内部添加 `.vt-panel` 容器和多个 `.vt-card` 占位卡片：
...
<aside class="sidebar">
  <div class="vt-panel sidebar-panel">
    ...
    <div class="panel-content">
      <div class="vt-card file-card">
        <div class="card-title">video_sample_01.mp4</div>
        <div class="card-meta vt-secondary">1920x1080 · 00:02:34</div>
      </div>
      <div class="vt-card file-card">
        <div class="card-title">video_sample_02.mp4</div>
        <div class="card-meta vt-secondary">3840x2160 · 00:05:12</div>
      </div>
      <div class="vt-card file-card">
        <div class="card-title">video_sample_03.mp4</div>
        <div class="card-meta vt-secondary">1280x720 · 00:01:45</div>
      </div>
    </div>
  </div>
</aside>
...
在 `<main class="workspace">` 内部添加视频预览区和时间轴区：
...
- [ ] **步骤 1：添加 Inspector 面板和占位内容**

在 `<aside class="inspector">` 内部添加 `.vt-panel` 容器、骨架文本和导出按钮：
...
在 `<style scoped>` 中添加三栏布局的 Flexbox 样式：
...
- 左侧固定 260px，右侧固定 340px，中间 flex-grow 撑满
...
- [ ] **步骤 1：添加 Sidebar 面板和卡片样式**

在现有 `<style scoped>` 中追加：
...
检查以下项目：
- ✅ 三栏布局没有塌陷，左侧 260px，右侧 340px，中间撑满
