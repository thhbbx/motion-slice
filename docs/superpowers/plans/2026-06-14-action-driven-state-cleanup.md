...
**重要改进**：
- `cleanupSingleModeData` 添加 try-catch 错误处理，防止动态 import 或清理过程异常导致状态不一致
- 错误仅记录日志，不阻断主流程（用户操作继续）
...
### 任务 5：重构 toggleVideoSelection 方法（含降维转换处理）

**文件：**
- 修改：`src/store/useVideoStore.ts:74-81`

- [ ] **步骤 1：重写 toggleVideoSelection 实现降维转换检测和分场景清理**

**关键改进**：不仅清理单个视频数据，还必须检测"模式降维转换"并执行完整的模式级清理。
...
**重要说明**：
- 场景 1 必须调用 `cleanupBatchModeData()` 彻底清空批量状态，防止残留视频的批量数据成为"孤儿状态"
- 场景 1 必须初始化单选状态（`currentTime` 和 `duration`），否则 Timeline 组件无法正常工作
- 场景 2 使用 `await` 确保清理完成后再清空状态，避免竞态问题
- 场景 3 维持批量模式，只需靶向删除该视频的数据
